...
enum tlsf_private
{
	...
	SL_INDEX_COUNT = (1 << SL_INDEX_COUNT_LOG2),             //2^4 = 16
	FL_INDEX_SHIFT = (SL_INDEX_COUNT_LOG2 + ALIGN_SIZE_LOG2),//6
	FL_INDEX_COUNT = (FL_INDEX_MAX - FL_INDEX_SHIFT + 1),    //12

	SMALL_BLOCK_SIZE = (1 << FL_INDEX_SHIFT),                //2^6 = 64
};
...
#define tlsf_insist(x) do { if (!(x)) { status--; } } while(0)
...
static void integrity_walker(void* ptr, size_t size, int used, void* user)
{
	...
	tlsf_insist(size == this_block_size);
