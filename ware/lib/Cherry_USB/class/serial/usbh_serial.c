...
static uint8_t *g_serial_iobuffer[CONFIG_USBHOST_MAX_SERIAL_CLASS] = {NULL};
...
static struct usbh_serial *usbh_serial_alloc(bool is_cdcacm)
{
    ...
    for (devno = 0; devno < CONFIG_USBHOST_MAX_SERIAL_CLASS; devno++) {
        if ((g_devinuse & (1U << devno)) == 0) {
            ...
            // ============== 新增：动态申请 iobuffer ==============
            if (g_serial_iobuffer[devno] == NULL) {
                uint32_t buf_size = USB_ALIGN_UP((USBH_SERIAL_RX2_NOCACHE_OFFSET + USBH_SERIAL_RX2_NOCACHE_SIZE), CONFIG_USB_ALIGN_SIZE);
                g_serial_iobuffer[devno] = (uint8_t *)malloc_bsc(buf_size);
                if (g_serial_iobuffer[devno] == NULL) {
                    g_devinuse &= ~(1U << devno);
                    USB_LOG_ERR("Fail to alloc serial iobuffer\r\n");
                    return NULL;
                }
            }
            // ===================================================
...
static void usbh_serial_free(struct usbh_serial *serial)
{
    ...
    // ============== 新增：释放动态申请的内存 ==============
    if (serial->rx_rb_pool) {
        free_bsc(serial->rx_rb_pool);
        serial->rx_rb_pool = NULL;
    }
    if (g_serial_iobuffer[devno]) {
        free_bsc(g_serial_iobuffer[devno]);
        g_serial_iobuffer[devno] = NULL;
    }
    // ===================================================
}
...
struct usbh_serial *usbh_serial_open(const char *devname, uint32_t open_flags)
{
    ...
    // ============== 新增：动态申请 ringbuffer 池 ==============
    if (serial->rx_rb_pool == NULL) {
        serial->rx_rb_pool = (uint8_t *)malloc_bsc(CONFIG_USBHOST_SERIAL_RX_SIZE);
        if (serial->rx_rb_pool == NULL) {
            USB_LOG_ERR("Fail to alloc serial rx_rb_pool\r\n");
            return NULL;
        }
    }
    // =========================================================
