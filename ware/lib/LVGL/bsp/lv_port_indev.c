...
static void mouse_read(lv_indev_drv_t * indev_drv, lv_indev_data_t * data)
{
    if (g_lvgl_input_disabled)// 如果LVGL输入被禁用，则不更新鼠标位置和状态 
	{
		...
		{
			last_L = g_key_L_M_RT;// 更新上一次左摇杆按下状态为当前状态
    ...
    /* --------- 1. 左摇杆：仅负责鼠标光标移动(方向) --------- */
    float move_x = (g_key_L_X * 0.06f) + (g_usb_joy_L_X * 0.06f) + usb_dx;
    float move_y = -(g_key_L_Y * 0.06f) - (g_usb_joy_L_Y * 0.06f) + usb_dy; 
    ...
    /* --------- 2. 右摇杆：仅负责触发点击/确定(按下)状态 --------- */
    int16_t combined_R_X = g_key_R_X + g_usb_joy_R_X;
    int16_t combined_R_Y = g_key_R_Y + g_usb_joy_R_Y;
    
    // 只要右摇杆向任意方向拨动超过死区（例如50），即视为“确定/鼠标左键按下”
    bool is_clicked = false;
    if (abs(combined_R_X) > 50 || abs(combined_R_Y) > 50 || (usb_btn & 0x01)) {
        is_clicked = true;
    }
