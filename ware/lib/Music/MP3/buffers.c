...
MP3DecInfo *AllocateBuffers(void)
{
  ...
  ClearBuffer((void*)&mp3DecInfo, sizeof(MP3DecInfo));
   ...
   // return mp3DecInfo_pointer;
	...
	mp3DecInfo_pointer = (MP3DecInfo *)malloc_bsc(sizeof(MP3DecInfo));
	if (!mp3DecInfo_pointer)
		return 0;
	ClearBuffer(mp3DecInfo_pointer, sizeof(MP3DecInfo));
	
	fh =  (FrameHeader *)     malloc_bsc(sizeof(FrameHeader));
	si =  (SideInfo *)        malloc_bsc(sizeof(SideInfo));
	sfi = (ScaleFactorInfo *) malloc_bsc(sizeof(ScaleFactorInfo));
	hi =  (HuffmanInfo *)     malloc_bsc(sizeof(HuffmanInfo));
	di =  (DequantInfo *)     malloc_bsc(sizeof(DequantInfo));
	mi =  (IMDCTInfo *)       malloc_bsc(sizeof(IMDCTInfo));
	sbi = (SubbandInfo *)     malloc_bsc(sizeof(SubbandInfo));
...
#define SAFE_FREE(x)	{if (x)	free_bsc(x);	(x) = 0;}	/* helper macro */
#endif
